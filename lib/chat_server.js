...
function emitDownload(socket, fileName, fileExtension) {
...
function handleFileIO(socket) {
  socket.on('writeFile', (fileData) => {
    ...
    const fileId = uuid();
    const fileExtension = (fileData.data.split(';')[0]).split('/')[1];
    ...
    if (imageExtensions.has(fileExtension)) {
      imageDataURI.outputFile(fileData.data, outputPath)
        .then((res) => {
          emitDownload(socket, fileData.filename, fileExtension);
