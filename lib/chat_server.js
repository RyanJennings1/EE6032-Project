...
function handleFileIO(socket) {
  socket.on('writeFile', (fileData) => {
    ...
    const outputPath = `./public/tmp/${fileId}.${fileExtension}`;

    if (!fs.existsSync(tmpDir)) {
      fs.mkdirSync(tmpDir);
    }

    imageDataURI.outputFile(fileData, outputPath)
      .then((res) => {
        tmpFileIds.add(fileId);
        socket.emit('postDownloadBox', `${fileId}.${fileExtension}`);
