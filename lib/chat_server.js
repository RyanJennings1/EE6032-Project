...
function handleEncryptedMessageBroadcasting(socket) {
  socket.on('message', (message) => {
    socket.broadcast.to(message.room).emit('otherUserEncryptedMessage', {
      text: `${message.text}`,
      userId: socket.id,
    });
  });
...
function handleEncryptionDisplayTurnOn(socket) {
  socket.on('turnOnEncryption', () => {
    socket.broadcast.to('Lobby').emit('initiateEncryptionDisplay');
  });
}

function handleEncryptionDisplayTurnOff(socket) {
  socket.on('turnOffEncryption', () => {
    socket.broadcast.to('Lobby').emit('stopEncryptionDisplay');
  });
}
...
exports.listen = (server) => {
  ...
  io.sockets.on('connection', (socket) => {
    ...
    if ((guestNumber - 1) <= 2) {
      ...
      handleEncryptedMessageBroadcasting(socket, nickNames);
